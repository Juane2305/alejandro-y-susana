...
export const Modal = ({
  ...
  return (
    <>
      ...
      >
        {nombreBoton || "Ver Datos Bancarios"}
      </button>

      {isOpen && createPortal(
        <div 
          className="fixed inset-0 flex justify-center items-center" 
          style={{ backgroundColor: 'rgba(0, 0, 0, 0.80)', zIndex: 99999 }}
          onClick={() => setIsOpen(false)}
        >
          <div className={`bg-white p-10 rounded-lg flex flex-col justify-center items-center border-2 ${borderModal}`} style={styleBorderModal} onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col justify-center text-center space-y-7 text-gray-900 w-full max-w-md ">
              ...
              <div className="w-full space-y-4 text-left text-gray-800">
                ...
                {(cbu || cvu) && (
                  <p className="text-lg flex justify-between items-center">
                    <span><span className="font-semibold">{cvu ? 'CVU' : 'CBU'}:</span> {cvu || cbu}</span>
                    <button onClick={() => copiarTexto(cvu || cbu)}>
